...
export const MovementListItemComponent: React.FC<Props> = (props) => {
  ...
  return (
    <div className={classes.row}>
      ...
      <span className={`${classes.dataCell} ${classes.alignRight} ${movementItem.amount < 0 ? classes.negativeValue : ""}`}>{movementItem.amount} €</span>
      <span className={`${classes.dataCell} ${classes.alignRight} ${movementItem.balance < 0 ? classes.negativeValue : ""}`}>{movementItem.balance} €</span>
    </div>
  );
};
